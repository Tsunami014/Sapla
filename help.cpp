...
"<h1><b>IMPORTANT THINGS</b></h1>"
    "<ul>"
        ...
        "<li>"
            ...
            "<p>Also, some note features may not work inside other note features</p>"
        ...
        "</li>"
        "<li>Making separate cards is preferred over making one card with randomness</li>"
    "</ul>"
...
"<h2><u>Argument usage</u></h2>"
    "<p>Arguments are specified as either %index (e.g. "+code("%1")+" for the first argument) or %name (when defined in an argument specifier) or "+code("%#")+" (for every argument separated by space (with spaces escaped; for use with "+code("{")+")).</p>"
...
"<h2><u>Equations</u></h2>"
    ...
    "<p>Equations are a string of variables, operations and numbers.</p>"
    "<p>The variables used must equate to floats otherwise it will fail.</p>"
    "<p>"+hl("x")+" is a special variable equating to the contents this is a suffix for..</p>"
    "<p>Usable symbols are + (addition) - (subtraction) * (multiplication) / (division) % (modulo)</p>"
    "<p>This means you cannot use the above symbols "+hl("(including %, use $ to use variables instead)")+" in variables inside equations (but you can get around this with macros)</p>"
    "<p>Failing equations output "+hl("???")+"</p>"
    "<p>Using () to escape the contents is a good idea.</p>"
    "<p>Here is an example which multiplies %1 by 3 "+code("%1&(x*3)%")+"</p>"
    "<p>You can also reference other variables, such as: "+code("%1&($a*2)%")+" when used with "+code("[a=x+1]")+"</p>"
"<h2><u>Loops</u></h2>"
